...
|---|---|---|---|---|---|---|
| 2026-05-24 | (pending) | Admin console UI | New admin-only Rule Coverage tab inside existing `/app/admin` renders the sanitized output of `/api/admin/instagram/rule-coverage-inspector`. Uses the existing authenticated API client; gated by the same `canViewUsers` check as Automation Stop Point; copies output as JSON. Diagnostics page route remains absent (`App.routes.test.js` still asserts it). | No backend or automation change | Frontend 184 passed; frontend production build passed | Local, deploy required |
