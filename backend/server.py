...
from app.services.instagram.rule_normalizer import (
    normalize_comment_dm_rule,
    _materialize_comment_dm_rule,
...
from app.services.instagram.account_resolver import (
    _instagram_context_from_account,
    _current_instagram_context,
    _account_scoped_query,
...
# Phase 2.18Z: _redact_secrets, _safe_provider_error_payload,
# classify_instagram_send_error, _detailed_send_result, and the
# PERMANENT/TRANSIENT_GRAPH_FAILURE_REASONS frozensets all live in
# backend/dm_classifier.py now. This file just re-exports them so the
# 19k-line monolith no longer carries pure helpers.
...
# Phase 2.18Z: password policy moved to backend/password_policy.py.
# Re-exported here so existing references work unchanged.
from password_policy import (
    PASSWORD_MIN_LENGTH,
    PASSWORD_MAX_LENGTH,
    enforce_password_policy as _enforce_password_policy,
)
...
async def getActiveInstagramAccount(user_id: str) -> dict:
    return await _get_active_instagram_account_impl(
        user_id,
        db=db,
        sync_user_instagram_account_doc=_sync_user_instagram_account_doc,
        parse_graph_datetime=_parse_graph_datetime,
...
async def _active_instagram_account_owner(instagram_account_id: str) -> Optional[dict]:
    return await _active_instagram_account_owner_impl(
        instagram_account_id,
        db=db,
        canonical_instagram_account_id=_canonical_instagram_account_id,
    )
