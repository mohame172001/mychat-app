...
const AutomationPhonePreview = ({
  ...
  const Avatar = ({ size = 'h-9 w-9' }) => (
    <div className={`${size} shrink-0 overflow-hidden rounded-full bg-slate-200`}>
      {accountAvatar ? (
        <img src={accountAvatar} alt="" loading="lazy" referrerPolicy="strict-origin-when-cross-origin" className="h-full w-full object-cover" />
      ) : (
            ...
            {previewTab === 'Post' && (
              <>
                ...
                <div className="aspect-square bg-slate-900">
                  {previewImage ? (
                    <img src={previewImage} alt="" loading="lazy" referrerPolicy="strict-origin-when-cross-origin" className="h-full w-full object-cover" />
                  ) : (
                      ...
                      <div className="flex gap-2 overflow-x-auto pb-1">
                        {(showAllMedia ? media : media.slice(0, 8)).map(item => {
                          ...
                          return (
                            ...
                            >
                              {thumb ? (
                                <img src={thumb} alt="" loading="lazy" referrerPolicy="strict-origin-when-cross-origin" className="h-full w-full object-cover" />
                              ) : (
            ...
            <Card key={a.id} className="rounded-lg border-slate-100 p-4 transition-shadow hover:shadow-md">
              <div className="flex flex-wrap items-center gap-4">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br from-pink-500 via-fuchsia-500 to-orange-400">
                  {thumb ? <img src={thumb} alt="" loading="lazy" referrerPolicy="strict-origin-when-cross-origin" className="h-full w-full object-cover" /> : <Zap className="h-6 w-6 text-white" />}
                </div>
