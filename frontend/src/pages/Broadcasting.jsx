...
import { Plus, Send, Calendar, Users, Eye, MousePointer, MoreVertical, Loader2 } from 'lucide-react';
...
const Broadcasting = () => {
  ...
  const handleCreate = async () => {
    if (!name || !message) { toast.error('Fill all fields'); return; }
    try {
  ...
  const handleSend = async (bid) => {
    try {
      ...
      // Poll until status changes to 'sent'
  ...
  const stats = { sent: list.filter(b => b.status === 'sent').length };
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex items-end justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="mt-1 text-slate-600">Send messages to segments of your audience.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="bg-slate-900 hover:bg-slate-800 text-white rounded-xl"><Plus className="w-4 h-4 mr-1.5" /> New Broadcast</Button>
          </DialogTrigger>
          <DialogContent className="rounded-2xl">
            <DialogHeader><DialogTitle className="font-display text-2xl">Create Broadcast</DialogTitle></DialogHeader>
            <div className="space-y-4 pt-2">
              <div className="space-y-2"><Label>Broadcast name</Label><Input placeholder="Black Friday Sale" value={name} onChange={e => setName(e.target.value)} className="rounded-xl h-11" /></div>
              <div className="space-y-2"><Label>Message</Label><textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Hi {{first_name}}! ✨..." className="w-full min-h-[120px] px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900" /></div>
              <div className="flex items-center gap-2 text-sm text-slate-600"><Users className="w-4 h-4" /> Will be sent to your subscribed contacts</div>
            </div>
            <DialogFooter>
      ...
      <div className="mt-8 grid sm:grid-cols-3 gap-4">
        ...
        <Card className="p-5 rounded-2xl border-slate-100">
          <div className="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center"><Eye className="w-5 h-5 text-emerald-600" /></div>
          <div className="mt-4 text-3xl font-extrabold font-display">62%</div>
          <div className="text-sm text-slate-500 mt-1">Avg. open rate</div>
        </Card>
        <Card className="p-5 rounded-2xl border-slate-100">
          <div className="w-10 h-10 rounded-xl bg-pink-50 flex items-center justify-center"><MousePointer className="w-5 h-5 text-pink-600" /></div>
          <div className="mt-4 text-3xl font-extrabold font-display">28%</div>
          <div className="text-sm text-slate-500 mt-1">Avg. click rate</div>
        </Card>
      </div>
        ...
        <div className="divide-y divide-slate-50">
          {list.map(b => (
            <div key={b.id} className="grid grid-cols-[1fr_auto] md:grid-cols-[1fr_120px_140px_120px_120px_40px] items-center gap-4 px-5 py-4 hover:bg-slate-50 transition-colors">
              ...
              <Badge className={`rounded-full capitalize ${statusColors[b.status]}`}>{b.status}</Badge>
